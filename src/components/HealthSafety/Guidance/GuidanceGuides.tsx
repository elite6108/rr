    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center z-50">
      <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl p-8 max-w-md w-full m-4">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Upload Guide</h2>
          <button
            onClick={onClose}
            className="text-gray-400 hover:text-gray-500 focus:outline-none"
          >
...
export function GuidanceGuides({ onBack }: GuidanceGuidesProps) {
  const [guides, setGuides] = useState<GuidanceGuide[]>([]);
  ...
  async function fetchThumbnailUrls(guides: GuidanceGuide[]) {
      ...
      if (guide.thumbnail_path) {
        try {
          const { data, error } = await supabase.storage
            ...
            .createSignedUrl(guide.thumbnail_path, 3600); // 1 hour expiry

          if (data && !error) {
            urlMap[guide.id] = data.signedUrl;
          }
        } catch (error) {
          console.error(`Error fetching thumbnail URL for guide ${guide.id}:`, error);
        }
      }
    }
  ...
  const handleViewPDF = async (guide: GuidanceGuide) => {
    try {
      const { data, error } = await supabase.storage
        .from('guidance-guides')
        .createSignedUrl(guide.file_path, 3600); // 1 hour expiry
  ...
  const handleUpload = async (title: string, pdfFile: File, thumbnailFile: File | null) => {
    ...
    try {
      // Upload PDF to guidance-guides bucket
      ...
      const { error: dbError } = await supabase.from('guidance_guides').insert([
        {
          title,
      ...
      // Reset form and refresh guides
      setShowUploadModal(false);
      fetchGuides();
    } catch (error) {
      console.error('Error uploading guide:', error);
      setError('Error uploading guide. Please try again.');
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h2 className="text-2xl font-semibold text-gray-900 dark:text-white">Guidance Documents</h2>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {loading ? (
          ...
          filteredGuides.map((guide) => (
            <div key={guide.id} className="bg-gray-50 dark:bg-gray-800 rounded-lg shadow-sm p-4 text-center flex flex-col justify-between border border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow">
              <div className="flex-grow flex flex-col items-center justify-center pt-4">
                {guide.thumbnail_path && thumbnailUrls[guide.id] ? (
                  <img
                    src={thumbnailUrls[guide.id]}
                    alt={guide.title}
