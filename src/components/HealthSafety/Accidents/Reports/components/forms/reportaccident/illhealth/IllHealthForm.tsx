...
import { generateIllHealthId } from '../utils/illHealthIdUtils';
import { useIllHealthFormSubmit } from '../hooks/useIllHealthFormSubmit';
import { useIllHealthFileUpload } from '../hooks/useIllHealthFileUpload';
import { IllHealthFormData, IllHealthFormProps } from '../types/IllHealthTypes';
...
export default function IllHealthForm({ onClose, initialData, isEditing = false }: IllHealthFormProps) {
  ...
  useEffect(() => {
    if (!isEditing) {
      const getNextId = async () => {
        setIsLoadingId(true);
        try {
          const newId = await generateIllHealthId();
            setAutoId(newId);
  ...
  const [formData, setFormData] = useState<IllHealthFormData>(() => {
  ...
  // Use custom hooks
  const { isSubmitting, submitted, error, handleSubmit, setError } = useIllHealthFormSubmit();
  const { uploadingFiles, handleFileUpload } = useIllHealthFileUpload(formData, setFormData, isEditing, initialData);
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-green-700 dark:text-green-300">
                <p>Your ill health report has been successfully {isEditing ? 'updated' : 'submitted'} and saved.</p>
              </div>
            </div>
          </div>
        </div>
