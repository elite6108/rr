...
import { generateFatalityId } from '../utils/fatalityIdUtils';
import { useFatalityFormSubmit } from '../hooks/useFatalityFormSubmit';
import { useFatalityFileUpload } from '../hooks/useFatalityFileUpload';
import { FatalityFormData, FatalityFormProps } from '../types/FatalityTypes';
...
export default function FatalityForm({ onClose, initialData, isEditing = false }: FatalityFormProps) {
  ...
  useEffect(() => {
    if (!isEditing) {
      const getNextId = async () => {
        setIsLoadingId(true);
        try {
          const newId = await generateFatalityId();
            setAutoId(newId);
  ...
  const [formData, setFormData] = useState<FatalityFormData>(() => {
  ...
  // Use custom hooks
  const { isSubmitting, submitted, error, handleSubmit, setError } = useFatalityFormSubmit();
  const { uploadingFiles, handleFileUpload } = useFatalityFileUpload(formData, setFormData, isEditing, initialData);
