...
import { UnsafeActionsFormData, UnsafeActionsFormProps } from '../types/UnsafeActionsTypes';
import { getNextUnsafeActionsId } from '../utils/unsafeActionsIdUtils';
import { refreshUnsafeActionsFileUrls } from '../utils/unsafeActionsFileUtils';
import { useUnsafeActionsFileUpload } from '../hooks/useUnsafeActionsFileUpload';
import { useUnsafeActionsFormSubmit } from '../hooks/useUnsafeActionsFormSubmit';
import { 
  UnsafeActionsDetailsStep, 
  UnsafeActionsDescriptionStep, 
  UnsafeActionsHealthSafetyStep, 
  UnsafeActionsRootCausesStep 
} from '../components/UnsafeActionsBasicSteps';
import { UnsafeActionsActionsStep } from '../components/UnsafeActionsActionsStep';
import { UnsafeActionsDocumentationStep } from '../components/UnsafeActionsDocumentationStep';

export default function UnsafeActionsForm({ onClose, initialData, isEditing = false }: UnsafeActionsFormProps) {
  ...
  useEffect(() => {
    if (!isEditing) {
      const loadId = async () => {
        setIsLoadingId(true);
        try {
          const newId = await getNextUnsafeActionsId();
          setAutoId(newId);
          setFormData(prev => ({ ...prev, autoId: newId }));
        } catch (error) {
          console.error('Error getting next ID:', error);
          const defaultId = 'UA-00001';
          setAutoId(defaultId);
  ...
  const [formData, setFormData] = useState<UnsafeActionsFormData>(() => {
    if (isEditing && initialData) {
      ...
      const formDataObject = {
        ...
        reportType: initialData.reportType || initialData.report_type || 'Unsafe Actions',
      ...
      if (fileUrlsFromDb) {
        ...
        if (fileUrls.length > 0) {
          ...
          const refreshUrls = async () => {
            try {
              const refreshedUrls = await refreshUnsafeActionsFileUrls(fileUrls);
  ...
  // Use custom hooks
  const { handleFileUpload, uploadingFiles } = useUnsafeActionsFileUpload();
  const { handleSubmit, isSubmitting, submitted, error } = useUnsafeActionsFormSubmit();
  ...
  const stepContent = [
    ...
    <UnsafeActionsRootCausesStep 
      ...
      isLoadingId={isLoadingId} 
    />,
    <UnsafeActionsActionsStep 
      key="actions" 
      formData={formData} 
      setFormData={setFormData} 
    />,
    <UnsafeActionsDocumentationStep 
      key="documentation" 
      formData={formData} 
      setFormData={setFormData} 
  ...
  return (
    <div className="unsafe-actions-form">
      <h1 className="text-xl font-bold mb-4 text-gray-900 dark:text-white">Unsafe Actions Report</h1>
      {submitted ? (
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-green-700 dark:text-green-300">
                <p>Your unsafe actions report has been successfully submitted and saved.</p>
              </div>
            </div>
          </div>
        </div>
