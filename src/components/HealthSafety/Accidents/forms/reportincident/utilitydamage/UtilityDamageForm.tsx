...
interface UtilityDamageFormProps {
  onClose?: () => void;
  initialData?: any;
  isEditing?: boolean;
}
...
const parseArrayField = (field: any): string[] => {
  ...
  if (typeof field === 'string') {
    try {
      ...
      // If it's not valid JSON, treat it as a single item or empty
...
export default function UtilityDamageForm({ onClose, initialData, isEditing = false }: UtilityDamageFormProps) {
  ...
  const [formData, setFormData] = useState<FormData>(() => {
    if (isEditing && initialData) {
      ...
      console.log('PPE specific debug:');
      console.log('- initialData.rootCausePpe:', initialData.rootCausePpe);
      console.log('- initialData.root_cause_ppe:', initialData.root_cause_ppe);
      console.log('- Type of rootCausePpe:', typeof initialData.rootCausePpe);
      console.log('- Type of root_cause_ppe:', typeof initialData.root_cause_ppe);
      console.log('- Is rootCausePpe array?', Array.isArray(initialData.rootCausePpe));
      console.log('- Is root_cause_ppe array?', Array.isArray(initialData.root_cause_ppe));
      
      console.log('Human Factors comparison:');
      console.log('- initialData.rootCauseHumanFactors:', initialData.rootCauseHumanFactors);
      console.log('- initialData.root_cause_human_factors:', initialData.root_cause_human_factors);
      console.log('- Type of rootCauseHumanFactors:', typeof initialData.rootCauseHumanFactors);
      console.log('- Type of root_cause_human_factors:', typeof initialData.root_cause_human_factors);
      console.log('- Is rootCauseHumanFactors array?', Array.isArray(initialData.rootCauseHumanFactors));
      console.log('- Is root_cause_human_factors array?', Array.isArray(initialData.root_cause_human_factors));
      ...
      const formDataObject = {
        ...
        serviceDescription: initialData.serviceDescription || initialData.service_description || '',
        serviceOwner: initialData.serviceOwner || initialData.service_owner || '',
        causeDamage: initialData.causeDamage || initialData.cause_damage || '',
        plantOwnerName: initialData.plantOwnerName || initialData.plant_owner_name || '',
        repairService: initialData.repairService || initialData.repair_service || '',
        repairDate: initialData.repairDate || initialData.repair_date || '',
        responsiblePerson: initialData.responsiblePerson || initialData.responsible_person || '',
        ...
        rootCausePpe: parseArrayField(initialData.rootCausePpe || initialData.rootCausePpe || initialData.root_cause_ppe),
      ...
      console.log('Final form data object debug:');
      console.log('- Final rootCausePpe:', formDataObject.rootCausePpe);
      console.log('- Final rootCauseHumanFactors:', formDataObject.rootCauseHumanFactors);
      console.log('- Are they equal?', JSON.stringify(formDataObject.rootCausePpe) === JSON.stringify(formDataObject.rootCauseHumanFactors));
  ...
  const handleSubmit = async () => {
    try {
      ...
      const dataToSubmit = {
          ...
          service_owner: formData.serviceOwner,
          cause_damage: formData.causeDamage,
          plant_owner_name: formData.plantOwnerName,
          repair_service: formData.repairService,
  ...
  const formSteps: Step[] = [
    ...
    { id: 'utility-damage', title: 'Utility Damage' },
  ...
  const stepContent = [
    ...
    // Step 3: Utility Damage
    <div key="utility-damage" className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Description of the service *</label>
        <textarea 
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Owner of service *</label>
        <input 
          type="text" 
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Cause of damage *</label>
        <select 
          value={formData.causeDamage} 
          onChange={e => setFormData({ ...formData, causeDamage: e.target.value })} 
          className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white"
          required
        >
          <option value="">Select cause of damage</option>
          <option value="Hand operated plant">Hand operated plant</option>
          <option value="Hand tools">Hand tools</option>
          <option value="Mechanical plant">Mechanical plant</option>
        </select>
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Plant owner's name / plant hire company name *</label>
        <input 
          type="text" 
          value={formData.plantOwnerName} 
          onChange={e => setFormData({ ...formData, plantOwnerName: e.target.value })} 
          className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" 
          required 
        />
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Who undertook the repair service *</label>
        <input 
          type="text" 
          value={formData.repairService} 
          onChange={e => setFormData({ ...formData, repairService: e.target.value })} 
          className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" 
          required 
        />
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Repair date *</label>
        <input 
          type="date" 
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Person responsible for the damage *</label>
        <input 
          type="text" 
    ...
    <div key="root-causes" className="space-y-6">
      ...
      <div>
        ...
        {/* Debug log for PPE rendering */}
        {console.log('PPE Debug - Current formData.rootCausePpe:', formData.rootCausePpe)}
        {console.log('PPE Debug - Type:', typeof formData.rootCausePpe)}
        {console.log('PPE Debug - Is array?', Array.isArray(formData.rootCausePpe))}
        <div className="flex flex-wrap gap-2">
