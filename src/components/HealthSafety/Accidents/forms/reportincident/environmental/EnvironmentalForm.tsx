...
export default function EnvironmentalForm({ onClose, initialData, isEditing = false }: EnvironmentalFormProps) {
  // Generate autoId on load with format ENV-XXXXX (where X is a number) or use existing one
  ...
  const handleFileRemove = (indexToRemove: number, fieldName: 'file_urls') => {
    setFormData(prev => ({
      ...prev,
  ...
  const stepContent = [
    ...
    <div key="environmental-incident" className="space-y-4">
      <div>
