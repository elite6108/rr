...
export default function DangerousOccurrenceForm({ 
  ...
  // Generate autoId on load with format DO-XXXXX (where X is a number)
    ...
    if (!isEditing) {
      const getNextId = async () => {
        ...
        const { data, error } = await supabase
            .from('accidents_dangerousoccurrence')
            .select('auto_id')
  ...
  const [formData, setFormData] = useState<FormData>(() => {
    ...
    // Preventive Measures
        ...
        reportType: initialData.reportType || initialData.report_type || 'Dangerous Occurrence',
    ...
    // Form specific fields
    autoId,
      reportType: 'Dangerous Occurrence',
  ...
  useEffect(() => {
    if (isEditing && initialData) {
      ...
      console.log('initialData.basic_cause_of_incident:', initialData.basic_cause_of_incident);
      console.log('initialData.basicCause:', initialData.basicCause);
      console.log('formData.basicCause:', formData.basicCause);
      console.log('initialData.source_of_hazard:', initialData.source_of_hazard);
      console.log('initialData.sourceOfHazard:', initialData.sourceOfHazard);
      console.log('formData.sourceOfHazard:', formData.sourceOfHazard);
              ...
              const newUrls = await Promise.all(
                fileUrls.map(async (url: any) => {
                  ...
                  if (pathMatch) {
                    ...
                    if (path.startsWith('accidents-dangerousoccurrence/')) {
                      path = path.replace('accidents-dangerousoccurrence/', '');
                      console.log('UseEffect: Cleaned path (removed bucket name):', path);
                    }
                    
                    console.log('UseEffect: Refreshing URL for path:', path);
                    const { data, error } = await supabase.storage
                      .from('accidents-dangerousoccurrence')
  ...
  const formSteps: Step[] = [
    ...
    { id: 'hse', title: 'HSE Documentation' },
  ];

  // Content for each step
  const stepContent = [
    // Step 1: ID, Report Type, Category
    ...
    <div key="incident-description" className="space-y-4">
      ...
      <div>
        ...
        <textarea 
          ...
          required 
        />
        <p className="text-xs text-gray-500 mt-1">In the case of an injury, state what the injured person was doing at the time and side of the body (left or right). (Where possible, take photographs of the general area but not of the injured persons.) In the case of an environmental incident, state the events that caused the incident (details of plant involved, photographs, wherever practicable, must be taken). In the case of damage, indicate if the damage is to permanent works, temporary works, plant, temporary buildings/contents, and employee's permanent effects.</p>
      </div>
    </div>,
    ...
    // Step 4: Root Causes (multi-selects as modern button style)
    ...
    <div key="actions" className="space-y-4">
      <div>
        ...
        <p className="text-xs text-gray-500 mt-1">Please outline below what has been done to prevent a similar incident from recurring in the future. Note: The details entered here are not designed to replace the detailed actions and recommendations arising out of a thorough incident investigation, but to indicate to interested parties what interim measures have been put in place, whilst such investigations are ongoing. In some circumstances, the information entered here may be sufficient to close out the report in full, such as near miss, minor accidents etc.</p>
        <br></br>
        <textarea 
    ...
    // Step 6: HSE Documentation
    <div key="hse" className="space-y-6">
  ...
  return (
    <div className="dangerous-occurrence-form">
      <h1 className="text-xl font-bold mb-4 text-gray-900 dark:text-white">Dangerous Occurrence Report</h1>
      {submitted ? (
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-green-700 dark:text-green-300">
                <p>Your dangerous occurrence report has been successfully submitted and saved.</p>
              </div>
            </div>
          </div>
        </div>
