...
interface OccupationalDiseaseFormProps {
  onClose?: () => void;
  initialData?: any;
  isEditing?: boolean;
}

export default function OccupationalDiseaseForm({ onClose, initialData, isEditing = false }: OccupationalDiseaseFormProps) {
  // Generate autoId on load with format OD-XXXXX (where X is a number) or use existing one
    ...
    if (!isEditing) {
      const getNextId = async () => {
        ...
        const { data, error } = await supabase
            .from('accidents_occupationaldisease')
            .select('auto_id')
  ...
  const [formData, setFormData] = useState<FormData>(() => {
    if (isEditing && initialData) {
      return {
        ...
        reportType: initialData.reportType || 'Occupational Disease',
    ...
    return {
      autoId,
      reportType: 'Occupational Disease',
              ...
              const newUrls = await Promise.all(
                fileUrls.map(async (url: any) => {
                  ...
                  if (pathMatch) {
                    ...
                    if (path.startsWith('accidents-occupationaldisease/')) {
                      path = path.replace('accidents-occupationaldisease/', '');
                      console.log('UseEffect: Cleaned path (removed bucket name):', path);
                    }
                    
                    console.log('UseEffect: Refreshing URL for path:', path);
                    const { data, error } = await supabase.storage
                      .from('accidents-occupationaldisease')
  ...
  const stepContent = [
    ...
    <div key="injured-person" className="space-y-4">
      ...
      <div>
        ...
        <textarea value={formData.wornPpe} onChange={e => setFormData({ ...formData, wornPpe: e.target.value })} rows={3} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" />
      </div>
    </div>,
  ...
  return (
    <div className="occupational-disease-form">
      <h1 className="text-xl font-bold mb-4 text-gray-900 dark:text-white">Occupational Disease Report</h1>
      {submitted ? (
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-green-700 dark:text-green-300">
                <p>Your occupational disease report has been successfully {isEditing ? 'updated' : 'submitted'} and saved.</p>
              </div>
            </div>
          </div>
        </div>
