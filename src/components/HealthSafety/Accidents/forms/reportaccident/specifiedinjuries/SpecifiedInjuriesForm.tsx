...
interface SpecifiedInjuriesFormProps {
  onClose?: () => void;
  initialData?: any;
  isEditing?: boolean;
}

export default function SpecifiedInjuriesForm({ onClose, initialData, isEditing = false }: SpecifiedInjuriesFormProps) {
  // Generate autoId on load with format SI-XXXXX (where X is a number) or use existing one
    ...
    if (!isEditing) {
      const getNextId = async () => {
        ...
        const { data, error } = await supabase
            .from('accidents_specifiedinjuries')
            .select('auto_id')
  ...
  const [formData, setFormData] = useState<FormData>(() => {
    if (isEditing && initialData) {
      return {
        ...
        reportType: initialData.reportType || 'Specified Injuries',
  ...
  useEffect(() => {
    if (isEditing && initialData) {
      ...
      // Check for both snake_case and camelCase versions, prioritize file_urls from database
      ...
      if (fileUrlsFromDb) {
        ...
        if (typeof fileUrlsFromDb === 'string') {
          try {
            ...
            console.log('UseEffect: Parsed file URLs from JSON string:', fileUrls);
          } catch (e) {
            console.log('UseEffect: Not JSON, treating as single URL string');
            // If not JSON, treat as a single URL or comma-separated URLs
            fileUrls = fileUrlsFromDb.split(',').map(url => url.trim()).filter(url => url.length > 0);
            console.log('UseEffect: Split file URLs from string:', fileUrls);
          }
        ...
        // Filter valid URLs
        fileUrls = fileUrls.filter((url: any) => url && typeof url === 'string' && url.trim().length > 0);
        ...
        if (fileUrls.length > 0) {
          ...
          // Then refresh the signed URLs if they look like Supabase URLs
          const refreshUrls = async () => {
            console.log('UseEffect: Starting to refresh signed URLs...');
            try {
              const newUrls = await Promise.all(
                fileUrls.map(async (url: any) => {
                  // Check if this looks like a Supabase signed URL that needs refreshing
                  ...
                  if (pathMatch) {
                    ...
                    if (path.startsWith('accidents-specifiedinjuries/')) {
                      path = path.replace('accidents-specifiedinjuries/', '');
                      console.log('UseEffect: Cleaned path (removed bucket name):', path);
                    }
                    
                    console.log('UseEffect: Refreshing URL for path:', path);
                    const { data, error } = await supabase.storage
                      .from('accidents-specifiedinjuries')
                  ...
                  // Return original URL if it doesn't need refreshing
                  return url;
                })
              );
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Combine newly uploaded files with existing file URLs
      const allFileUrls = [...formData.file_urls, ...fileUrls];
  ...
  const stepContent = [
    ...
    <div key="details" className="space-y-4">
      <div>
        ...
        <input type="text" value={isLoadingId ? "Loading..." : formData.autoId} readOnly disabled className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" />
      </div>
      <div>
    ...
    <div key="description" className="space-y-4">
      ...
      <div>
        ...
        <input type="date" value={formData.incidentDate} onChange={e => setFormData({ ...formData, incidentDate: e.target.value })} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" required />
      </div>
      <div>
      ...
      {formData.timeLost && (
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="date" value={formData.timeLostStartDate} onChange={e => setFormData({ ...formData, timeLostStartDate: e.target.value })} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" />
          </div>
          <div>
            ...
            <input type="date" value={formData.timeLostEndDate} onChange={e => setFormData({ ...formData, timeLostEndDate: e.target.value })} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" />
          </div>
        </div>
      )}
      <div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded shadow-lg w-full max-w-md relative">
            ...
            <div className="mb-2">
              ...
              <input type="date" value={actionDraft.dueDate} onChange={e => setActionDraft({ ...actionDraft, dueDate: e.target.value })} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" required />
            </div>
            <div className="mb-2">
  ...
  return (
    <div className="specified-injuries-form">
      <h1 className="text-xl font-bold mb-4 text-gray-900 dark:text-white">Specified Injuries Report</h1>
      {submitted ? (
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-green-700 dark:text-green-300">
                <p>Your specified injuries report has been successfully {isEditing ? 'updated' : 'submitted'} and saved.</p>
              </div>
            </div>
          </div>
        </div>
