...
interface FormData {
  ...
  file_urls: string[];
  fileUrls?: string[]; // For backward compatibility
  files?: any[]; // For backward compatibility
  file_url?: string; // For backward compatibility
}

export default function IllHealthForm({ 
  onClose,
  initialData,
  isEditing = false
}: { 
  ...
  // Generate autoId on load with format IH-XXXXX (where X is a number) or use existing one
  ...
  const [formData, setFormData] = useState<FormData>(() => {
    if (isEditing && initialData) {
      return {
        ...
        reportType: initialData.reportType || 'Ill Health',
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Combine existing file URLs with new ones
  ...
  return (
    <div className="ill-health-form">
      <h1 className="text-xl font-bold mb-4 text-gray-900 dark:text-white">Ill Health Report</h1>
      {submitted ? (
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-green-700 dark:text-green-300">
                <p>Your ill health report has been successfully submitted and saved.</p>
              </div>
            </div>
          </div>
        </div>
