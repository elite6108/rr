...
interface SevenDayIncapacitationFormProps {
  onClose?: () => void;
  initialData?: any;
  isEditing?: boolean;
}
              ...
              const newUrls = await Promise.all(
                fileUrls.map(async (url: any) => {
                  ...
                  if (pathMatch) {
                    ...
                    if (path.startsWith('accidents-sevendayincapacitation/')) {
                      path = path.replace('accidents-sevendayincapacitation/', '');
                      console.log('UseEffect: Cleaned path (removed bucket name):', path);
                    }
                    
                    console.log('UseEffect: Refreshing URL for path:', path);
                    const { data, error } = await supabase.storage
                      .from('accidents-sevendayincapacitation')
