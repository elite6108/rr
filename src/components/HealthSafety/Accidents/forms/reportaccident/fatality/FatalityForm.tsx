...
interface FatalityFormProps {
  onClose?: () => void;
  initialData?: any;
  isEditing?: boolean;
}

export default function FatalityForm({ onClose, initialData, isEditing = false }: FatalityFormProps) {
  // Generate autoId on load with format FT-XXXXX (where X is a number) or use existing one
  ...
  const [formData, setFormData] = useState<FormData>(() => {
    if (isEditing && initialData) {
      return {
        ...
        reportType: initialData.reportType || 'Fatality',
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Combine existing file URLs with any new uploads (if any)
      const allFileUrls = [...formData.file_urls];
...
time_lost_end_date: formData.timeLostEndDate || null,
  ...
  return (
    <div className="fatality-form">
      <h1 className="text-xl font-bold mb-4 text-gray-900 dark:text-white">Fatality Report</h1>
      {submitted ? (
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-green-700 dark:text-green-300">
                <p>Your fatality report has been successfully {isEditing ? 'updated' : 'submitted'} and saved.</p>
              </div>
            </div>
          </div>
        </div>
