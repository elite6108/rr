import { DSEAssessment } from '../types';
import { generateDSEPDF } from '../../../../utils/pdf/dse/dsePDFGenerator';

export const createPdfViewerHtml = (pdfDataUrl: string, filename: string): string => {
  return `
  <!DOCTYPE html>
  <html>
  <head>
...
export const createIosPdfViewerHtml = (pdfUrl: string, filename: string): string => {
  return `
  <!DOCTYPE html>
  <html>
  <head>
...
export const showLoadingInNewWindow = (newWindow: Window): void => {
...
export const generateFormattedFilename = (fullName: string): string => {
  return `DSE-Assessment-${fullName.replace(/\s+/g, '-')}-${new Date().toISOString().split('T')[0]}.pdf`;
};
...
export const handlePdfGeneration = async (
  ...
  try {
    ...
    if (!newWindow) {
      onError('Please allow popups for this site to view PDFs');
      return;
    }
    
    // Show loading state in the new window
    showLoadingInNewWindow(newWindow);
    ...
    const formattedFilename = generateFormattedFilename(assessment.full_name);

    // Check if window is still open
    if (newWindow.closed) {
      onError('PDF window was closed. Please try again.');
      return;
    }
    ...
    onError('Error generating DSE Assessment PDF. Please try again.');
  } finally {
    onComplete();
  }
};
