...
interface CPPHazardImportModalProps {
  show: boolean;
  onClose: () => void;
...
export function CPPHazardImportModal({ show, onClose, onImportHazards, onImportHazard }: CPPHazardImportModalProps) {
  const [previousCPPs, setPreviousCPPs] = useState<PreviousCPP[]>([]);
  const [previousRiskAssessments, setPreviousRiskAssessments] = useState<PreviousRiskAssessment[]>([]);
  const [loadingData, setLoadingData] = useState(false);
  const [selectedSource, setSelectedSource] = useState<string | null>(null);
  const [selectedSourceType, setSelectedSourceType] = useState<'cpp' | 'risk_assessment' | null>(null);
  ...
  const fetchPreviousData = async () => {
    ...
    try {
      ...
      // Fetch previous CPP forms with hazards
      const { data: cppData, error: cppError } = await supabase
        .from('cpp')
        .select('id, cpp_number, front_cover, created_at, hazards')
        .eq('user_id', user.id)
        .not('hazards', 'is', null)
      ...
      // Filter out CPPs that don't have hazards or have empty hazards array
      const validCPPs = (cppData || []).filter(cpp => 
        cpp.hazards && 
        Array.isArray(cpp.hazards) && 
        cpp.hazards.length > 0
      );

      setPreviousCPPs(validCPPs);

      // Fetch previous Risk Assessments with hazards
      const { data: riskAssessmentData, error: raError } = await supabase
      ...
      alert('Failed to fetch previous documents');
    } finally {
      setLoadingData(false);
    }
  };

  const handleSourceSelect = (sourceId: string, sourceType: 'cpp' | 'risk_assessment') => {
  ...
  const importAllHazards = async () => {
    if (!selectedSource || !selectedSourceType) return;

    setImportLoading(true);
    try {
      let selectedDocument: PreviousCPP | PreviousRiskAssessment | undefined;
      let documentName: string;

      if (selectedSourceType === 'cpp') {
        selectedDocument = previousCPPs.find(cpp => cpp.id === selectedSource);
        documentName = selectedDocument ? `CPP ${selectedDocument.cpp_number}` : 'Unknown CPP';
      } else {
        selectedDocument = previousRiskAssessments.find(ra => ra.id === selectedSource);
        documentName = selectedDocument ? selectedDocument.name : 'Unknown Risk Assessment';
      }

      if (!selectedDocument || !selectedDocument.hazards) {
        throw new Error('Selected document not found or has no hazards');
      }

      // Import hazards with new IDs to avoid conflicts
      const importedHazards = selectedDocument.hazards.map(hazard => ({
        ...hazard,
      ...
      // Show success modal
      setSuccessMessage(`Successfully imported ${importedHazards.length} hazards from ${documentName}`);
  ...
  const importSpecificHazard = async (hazardId: string) => {
    ...
    try {
      ...
      const hazardToImport = selectedDocument.hazards.find(h => h.id === hazardId);
      if (!hazardToImport) {
      ...
      // Show success modal
      setSuccessMessage(`Successfully imported hazard "${hazardToImport.title}" from ${documentName}`);
      setShowSuccessModal(true);
  ...
  const getSelectedHazards = () => {
    ...
    if (selectedSourceType === 'cpp') {
      return previousCPPs.find(cpp => cpp.id === selectedSource)?.hazards || [];
    } else {
      return previousRiskAssessments.find(ra => ra.id === selectedSource)?.hazards || [];
    }
  };
    ...
    <>
      ...
      <div className="absolute z-10 mt-1 w-full sm:w-80 bg-white border border-gray-300 rounded-md shadow-lg">
        <div className="p-4 space-y-4">
          <div className="text-sm font-medium text-gray-700">Select Document to Import From</div>
          
          {loadingData ? (
            <div className="text-sm text-gray-500">Loading previous documents...</div>
          ) : (previousCPPs.length === 0 && previousRiskAssessments.length === 0) ? (
            <div className="text-sm text-gray-500">No previous documents with hazards found</div>
          ) : (
            <>
              <div className="max-h-48 overflow-y-auto space-y-3">
                {/* Previous CPP Forms */}
                {previousCPPs.length > 0 && (
                  <div>
                    <div className="text-xs font-semibold text-gray-600 uppercase tracking-wide mb-2">
                      Previous CPP Forms
                    </div>
                    <div className="space-y-2">
                      {previousCPPs.map((cpp) => (
                        <button
                          key={`cpp-${cpp.id}`}
                          type="button"
                          onClick={() => handleSourceSelect(cpp.id, 'cpp')}
                          className={`w-full text-left p-2 hover:bg-gray-50 rounded cursor-pointer border ${
                            selectedSource === cpp.id ? 'border-indigo-500 bg-indigo-50' : 'border-transparent'
                          }`}
                        >
                          ...
                          <div className="text-xs text-gray-500 truncate">
                            {cpp.front_cover?.projectName || 'No project name'} • {cpp.hazards.length} hazards
                          </div>
                    ...
                    <div className="space-y-2">
                      {previousRiskAssessments.map((ra) => (
                        <button
                          key={`ra-${ra.id}`}
                          type="button"
                          onClick={() => handleSourceSelect(ra.id, 'risk_assessment')}
                          className={`w-full text-left p-2 hover:bg-gray-50 rounded cursor-pointer border ${
                            selectedSource === ra.id ? 'border-indigo-500 bg-indigo-50' : 'border-transparent'
                          }`}
                        >
                          <div className="text-sm font-medium text-gray-900 truncate">
                            {ra.name}
              ...
              {showImportOptions && selectedSource && selectedSourceType && (
