...
interface AsbestosArrangementsProps {
  onSectionSelect: (section: string) => void;
  selectedSection: string | null;
}

export const ASBESTOS_SECTIONS = {
  title: 'Asbestos Containing Materials',
  sections: [
    'Asbestos Management Plan',
    'Non-Licensed Work with Asbestos',
    'We do not work with ACMS'
  ]
};

export function AsbestosArrangements({ onSectionSelect, selectedSection }: AsbestosArrangementsProps) {
  return (
    <div className="space-y-2">
      <h4 className="text-sm font-medium text-gray-900 dark:text-white">{ASBESTOS_SECTIONS.title}</h4>
      <div className="space-y-1 ml-4">
        {ASBESTOS_SECTIONS.sections.map((section) => (
          <button
            key={section}
