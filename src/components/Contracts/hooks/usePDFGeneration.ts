...
export function usePDFGeneration() {
  const handleGeneratePDF = async (
    ...
    try {
      ...
      const pdfDataUrl = await generateContractPDF({
        contract: contractForPDF,
        companySettings: DEFAULT_COMPANY_SETTINGS
      });
      
      // Format filename
      const formattedDate = new Date(contract.contract_date).toISOString().split('T')[0];
      const customerName = contract.customer?.company_name || contract.customer?.customer_name || 'Client';
      const formattedFilename = `Contract-${customerName}-${formattedDate}.pdf`;
      ...
      if (isIOS) {
        ...
        <body>
          ...
          <div class="pdf-view">
            <embed id="pdf-embed" type="application/pdf" src="${pdfDataUrl}" width="100%" height="100%" />
          </div>
          <script>
